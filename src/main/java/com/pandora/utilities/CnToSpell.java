...
public class CnToSpell {
	...
	public static String getFullSpell(String cnStr)
	{
		...
		for (int i = 0, Len = chars.length; i < Len; i++) {
			...
			if (ascii.length() == 0) { // 取ascii时出错
				...
				//-----去除多音字------如果需要输出多音字，可以注释一下代码
				if(spell != null)
				{
					String[] tempList = spell.split(",");
	...
	//	for (int i = 0, Len = chars.length; i < Len; i++) {
		...
		//	System.out.println(ascii.length());
			if (ascii.length() == 1) { // 取ascii时出错
				...
				String spell = null;
				String spellTemp = getSpellByAscii(ascii);
		//		System.out.println("spellTemp:"+spellTemp);
				if(spellTemp != null)
				{
					spell = spellTemp.substring(0, 1);
				}
	...
	public static String GetInitialLetter(String cnStr)
	{
		if (null == cnStr || "".equals(cnStr.trim())) {
			return cnStr;
		}
